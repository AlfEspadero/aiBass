...
pub async fn acquisition_task(
	mut i2c: ImuI2c,
	pitch_tx: Sender<'static, embassy_sync::blocking_mutex::raw::ThreadModeRawMutex, PitchInput, 4>,
) {
	let mut whoami = [0u8; 1];
	match i2c
		.write_read(LSM6DSL_ADDR, &[REG_WHO_AM_I], &mut whoami)
		.await
	{
	...
	loop {
		...
		if let Err(e) = i2c
			.write_read(LSM6DSL_ADDR, &[REG_STATUS], &mut status)
			.await
		{
		...
		if let Err(e) = i2c
			.write_read(LSM6DSL_ADDR, &[REG_OUTX_L_XL], &mut raw)
			.await
		{
		...
		if let Some(input) = build_pitch_input(&ring)
			&& let Err(_e) = pitch_tx.try_send(input)
		{
